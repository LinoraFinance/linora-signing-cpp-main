...
std::string Account::getJwtToken( const std::string& url ) const
{
    ...
    try
    {
        ...
        std::string accountHeader = "linora-STARKNET-ACCOUNT: %1";
        {
            std::ostringstream s;
            s << address;
        ...
        std::string signatureHeader = "linora-STARKNET-SIGNATURE: [\"%1\", \"%2\"]";
        replace( &signatureHeader, "%1", signature.first );
        replace( &signatureHeader, "%2", signature.second );

        std::string hashHeader = "linora-STARKNET-MESSAGE-HASH: %1";
        replace( &hashHeader, "%1", hash );

        std::string nowHeader = "linora-TIMESTAMP: %1";
        std::string strNow = std::to_string( now );
        replace( &nowHeader, "%1", strNow );

        std::string expiryHeader = "linora-SIGNATURE-EXPIRATION: %1";
